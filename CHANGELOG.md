...
### Added
- Top.gg server count posting. RaidPresence was accepted onto Top.gg on 2026-08-26 but no code ever posted a count, so the listing showed **0 servers** — and Top.gg ranks and filters listings by that number, so the zero was costing placement and click-through for as long as it stood. The count comes from `client.guilds.cache.size`, deliberately not from the database: `Guild` retains a row for every server that ever installed the bot (135 rows against 56 live installs), so a naive count would have inflated the listing by roughly 2.5x
- Top.gg vote webhook. `POST /topgg/vote` verifies a shared secret in constant time and records each vote into the new `TopggVote` table. Top.gg only exposes a rolling 12-hour "has this user voted" check and keeps no queryable history, so a vote not captured as it arrives is gone permanently — storing them is what makes votes rewardable and measurable later. Dashboard test deliveries are recorded as `type = 'test'` rather than discarded, so a test is visibly a test instead of silently missing
- Both Top.gg integrations are **fully env-gated**. Without `TOPGG_TOKEN` the bot logs one informational line and creates no timer and no request; without `TOPGG_WEBHOOK_SECRET` it opens no HTTP port at all. A deployment with neither variable set behaves exactly as it did before. `docs/guides/TOPGG-INTEGRATION.md` documents where to obtain each credential, and repeatedly distinguishes the **Token** (issued by Top.gg, authenticates the outgoing server-count post) from the **Authorization** secret (chosen by you, verifies incoming webhooks) — they live on the same dashboard page and swapping them silently breaks both directions
- CI on pull requests. Until now the only workflow triggered on `v*` tags, so no pull request ever reported a check and every merge into `main` was unverified — the auto-archive fix below sat open for three weeks showing "no checks reported". The new `ci.yml` runs `npm ci`, `prisma generate`, `npm run lint` and the full Jest suite on every pull request to `main`, plus weekly on a schedule and on manual dispatch. It builds and publishes nothing; `ci-cd.yml` remains the tag-only deploy chain, untouched
- The Jest step calls `npx jest` directly. `npm test` is mapped to `tsc --noEmit` in `package.json`, so the suite does not run behind it — which is why the pre-existing `npm test` step in `ci-cd.yml` had never actually exercised the 1200+ tests it appeared to

### Fixed
- Auto-archive honours the `raid.archive` entitlement. The scheduler's auto-archive is the background twin of `/raid archive`, but it never checked the tier, so a guild whose trial had lapsed kept archiving forever and never saw the upsell every manual path shows. It now fails closed — an unreadable tier withholds the feature rather than granting it — and skips quietly, since a scheduler pass runs every two minutes and has no interaction to reply to. The raid still closes normally; only the archiving is withheld. Measured on production before merging: 2 guilds are affected, both already effectively FREE since their trials expired on 2026-08-27 and neither having run a raid in over 60 days. The one paying guild keeps the feature
- `premiumTier` is reset when premium expires. Nothing ever wrote the column back down when a trial lapsed — `grantTrialIfEligible()` stamped `PREMIUM` and that was the last write it received. Only `getTier()` accounted for expiry, at read time, via `premiumExpiresAt`. So the runtime was correct while the stored column was wrong, and every read that bypassed `getTier()` — direct SQL, dashboards, analytics — counted lapsed trials as paying customers. On 2026-09-01 the column reported **56 live premium guilds where the truth was 14**. An hourly reaper now corrects it, excluding any guild with a linked paid `entitlementId` unconditionally: a paid subscription with a past expiry is a renewal question for the provider, not an expiry, and this job must never be the thing that downgrades a paying customer
- **The reaper changes no user-visible behaviour.** `getTier()` has been returning FREE for all 42 affected guilds all along, so every runtime gate — features, team limits, the weekly raid cap — has already been treating them as free tier. This only makes the stored column agree with the behaviour that was already live

### Changed
- `docs/VERSION.md` documents what a `v*` tag actually does: build, publish to ghcr, Watchtower pull within 300s, container replaced, and `prisma migrate deploy` run against the production database — with no approval gate and no automated rollback. It previously presented tagging as an ordinary release step. Its stale duplicate of the release history (which still called v0.1.0 current while production ran v0.8.3) is gone; `CHANGELOG.md` is now the single source of truth
- `ROADMAP.md` brought up to the real state — Phase 2 moved off "Planning" (recurring raids, post-raid nudge and guided creation have shipped), Phase 3 credited with multi-team and install-base measurement, and priorities reordered to put activation and discovery first: 43 of 56 live guilds (77%) have never created a raid
- Merged branches pruned from the repository. Branches with unmerged work were left untouched

### Migration
- `20260901190000_topgg_votes` creates the `TopggVote` table and its two indexes. **Purely additive** — no existing table, column, constraint or row is touched, so it cannot affect current behaviour and rolling back is just dropping the table. Every statement is guarded and re-runnable. A fresh verified `pg_dump` was taken before the schema change (`raidpresence-20260901-183759.sql.gz`, 135 guilds) on top of the nightly at 03:15

### Notes
- The vote webhook is the first externally reachable surface the bot has ever had. It is disabled unless `TOPGG_WEBHOOK_SECRET` is set, an unset secret rejects every request rather than allowing them, the `Authorization` comparison is constant-time, neither the configured nor the supplied secret is ever logged, bodies are capped at 16 KB, and every path other than `POST /topgg/vote` returns 404. Top.gg requires a public HTTPS URL, so the port belongs behind the existing TLS-terminating reverse proxy — the `ports:` block in `docker-compose.yml` is deliberately left commented, and merging opens no host port
