...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(0 as any) as SlashCommandBuilder,
...
async function handleAttendanceCommand(interaction: ChatInputCommandInteraction) {
  ...
  const embed = formatAttendanceEmbed(player.username, playerStats, roleDistribution, history, period, guildData.language || 'en');
...
async function handleNotesCommand(interaction: ChatInputCommandInteraction) {
  ...
  const embed = formatRaidNotesEmbed(raid.description || 'Raid', raid.raidDate, notes, raid.guild.language || 'en');
...
async function handleSearchCommand(interaction: ChatInputCommandInteraction) {
  ...
  const results = await searchArchive({ guildId: interaction.guild.id, query, startDate });
